...
export BENCH_SUBTREES="${BENCH_SUBTREES:-10}"
export BENCH_COMMITS="${BENCH_COMMITS:-50}"
...
for cmd in status diff fetch; do
  args=(--warmup 1 --runs "$runs" --style none --export-markdown "$table"
    -n this "$head_entrypoint $cmd")
  [[ -n "$compare_entrypoint" ]] && args+=(-n compared "$compare_entrypoint $cmd")
