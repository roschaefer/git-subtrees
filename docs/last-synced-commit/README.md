...
consulted, and nothing depends on S, U or M, so it works the same for
...
| 2 | Cut `feature-1`; remote has no such branch | S1 / M1 (inherited) | `missing-at-head`, **unchanged** vs base `main` | Nothing under `vendor/a` differs from the merge base with `main`, so `push` skips it. |
| 3 | Commit a change under `vendor/a` | S1 / M1 | `missing-at-head`, **changed** vs base `main` | `vendor/a` differs from the merge base, so `push` would create `feature-1`. |
| 4 | `subtree push` creates remote `feature-1` | S1 / M1 (unchanged) | `up-to-date` | The remote branch exists now, so the same-name comparison applies, and it equals local. Note the sync point did not move. |
| 5 | Another branch changes `vendor/a` too; merge it into `feature-1` | S1 / M1 | `diverged` | See limitation 1. |
| 6 | Remote `feature-1` is deleted | S1 / M1 | `missing-at-head`, **changed** vs base `main` | Back to the base-branch case: `feature-1` still differs from `main`. |
...
| 8 | Merge `main` into `feature-1` | S2 / M2 (inherited) | `missing-at-head`, **changed** vs base `main` | The merge brought S2 in. `feature-1` still has changes relative to the merge base with `main`. |
...
1. **After a push, further local changes look `diverged`.** This is a
   ...
   *you* pushed to it), and local differs from M, so the tool sees two-sided
